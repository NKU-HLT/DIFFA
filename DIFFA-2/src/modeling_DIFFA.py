...
from src.origin_generate import generate as origin_generate
from src.dataloader import build_chat_prompt
from src.fast_dllm.diffa_generate import generate as fast_dllm_generate
...
class QformerConnector(torch.nn.Module):
    ...
    def forward(self, encoder_hidden_states):
        ...
        output = (layer_prompt_outputs * norm_weights).sum(dim=2) # (b, prompt_size, d_model)
...
class DIFFAModel(PreTrainedModel):
    ...
    def get_speech_features(self, speech_values, speech_attention_mask, stage=1):
        ...
        if stage == 1:
            print("==== get_speech_features by stage 1 ===")
    ...
    def generate(
        ...
        **kwargs
        ...
        if accelerate == "fast_dllm":
            print("generate with fast-dllm")
            out,_ = fast_dllm_generate(self, input_ids, audio_info_lengths=audio_info_lengths, speech_embeds=speech_embeds, steps=steps, gen_length=gen_length, block_length=block_length,factor=1.0)
        else:
            print("generate with origin LLaDA")
            out = origin_generate(self, input_ids, audio_info_lengths=audio_info_lengths, speech_embeds=speech_embeds, steps=steps, gen_length=gen_length, block_length=block_length, temperature=temperature, cfg_scale=cfg_scale, remasking=remasking, mask_id=mask_id)
        # pdb.set_trace()
